...
from app.models import Eventos, Usuario, Inscricoes, Tags
...
def visualizar(id):       
    if request.method == 'GET':
        ...
        return render_template("event_template.html",evento = evento, criador_nome = criador.nome, total_inscritos=total_inscritos, vencimento=vencimento)
...
def editar(id):
    ...
    if request.method == 'GET':
        return render_template('editar.html',evento=evento)
            ...
            evento.tipo = request.form['tipoForm']
            ...
            limite = request.form['limiteForm']
            evento.link = request.form['linkForm']

            # Ajuste de lógica do limite
            if limite == '':
                limite = 0
...
def toggle_favorito(id):
    ...
    if current_user.favoritos.filter_by(id=evento.id).first():
        current_user.favoritos.remove(evento)
        db.session.commit()
        return redirect(url_for('home.homepage'))
    else:
        current_user.favoritos.append(evento)
        db.session.commit()
        return redirect(url_for('home.homepage'))
